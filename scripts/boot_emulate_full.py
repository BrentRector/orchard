#!/usr/bin/env python3
"""
Full boot emulation of Apple Panic from WOZ disk image.
Traces through the entire boot process:
  P6 ROM -> $0801 -> stage 2 -> load sectors 0-9 to $B600-$BFFF
  -> JMP $B700 -> load tracks 1-5 to $0800+ -> JSR $1000
  -> load tracks 6-13 to $4000+ -> JMP $4000

Saves complete memory dumps at each stage.
"""
import sys
...
def main():
    ...
    for i in range(max_instr):
        ...
        if pc == 0x0343 and cpu.mem[0x3E] == 0x00 and cpu.mem[0x3F] == 0xB7:
            ...
            # Save stage 1 output
            with open("E:/Apple/emu_stage1_B600.bin", 'wb') as f:
                f.write(cpu.mem[0xB600:0xC000])

        # ---- Reached $B700 ----
        if pc == 0xB700 and not reached_B700:
            reached_B700 = True
            print(f"[{cpu.exec_count:9d}] *** Reached $B700 - Secondary loader ***")
        ...
        if pc == 0x1000 and reached_B700 and not reached_1000:
            ...
            with open("E:/Apple/emu_tracks1_5.bin", 'wb') as f:
        ...
        if pc == 0x4000 and reached_B700:
            if not reached_4000:
                ...
                with open("E:/Apple/emu_full_boot_memory.bin", 'wb') as f:
                    f.write(cpu.mem)
                print(f"  Full 64K memory saved to emu_full_boot_memory.bin")

                # Save game area
                with open("E:/Apple/emu_game_4000_A7FF.bin", 'wb') as f:
                ...
                with open("E:/Apple/emu_full_boot_B600.bin", 'wb') as f:
                    f.write(cpu.mem[0xB600:0xC000])
                print(f"  $B600-$BFFF saved to emu_full_boot_B600.bin")

                # Save intermediate code area
                with open("E:/Apple/emu_0800_3FFF.bin", 'wb') as f:
