#!/usr/bin/env python3
"""
Analyze raw nibble data from WOZ2 tracks to understand copy protection schemes.

Reads a WOZ2 disk image and searches each track's nibble stream for standard
and non-standard address/data field prologues (D5 AA 96, D5 AA AD, D5 AA B5,
etc.). Decodes any standard 4-and-4 address fields found and reports the byte
that follows each D5 marker to reveal custom protection patterns.

Usage:
    python woz_analyze.py

    Default path (override by editing APPLE_PANIC):
        woz_path - apple-panic/Apple Panic - Disk 1, Side A.woz

Output:
    For each of the first 14 tracks, prints prologue pattern match counts,
    D5-follower frequency distribution, address field decodes (volume, track,
    sector), and the first 100 nibbles of the track.
"""
import struct
...
def extract_bits(track_data, bit_count):
    """Convert packed byte array to a list of individual bits (MSB first)."""
    bits = []
...
def find_nibbles(bits):
    """Convert a bit stream to disk nibbles (bytes with high bit set).

    Simulates the Apple II disk controller shift register: bits shift in
    from the right and a byte is complete when bit 7 becomes set.
    """
    nibbles = []
    current = 0
    for b in bits:
...
def main():
    """Scan all tracks for prologue patterns and decode address fields."""
