...
Goes beyond the systematic approach of woz_53calibrate.py by trying unusual
packing schemes: raw bit concatenation (MSB/LSB), P5A ROM standard layout
(primary at [0:256], secondary at [256:410]), reversed P5A layout, and
interleaved secondary mappings with varying interleave factors. Also tests
"upper 5 bits only" (no secondary reconstruction) and performs sliding-window
partial matching and XOR key analysis against the cracked version.

Usage:
    python woz_53brute.py
...
Output:
    Byte-0 and first-bytes preview for each reconstruction method, partial
    match percentages (8-byte sliding window), and XOR key analysis results.
"""
import struct
...
def main():
    """Try brute-force 5-and-3 reconstruction methods and compare against cracked image."""
