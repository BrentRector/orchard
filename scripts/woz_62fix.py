#!/usr/bin/env python3
"""
Test correct 6-and-2 decode using multiple reconstruction algorithms.

Compares five different methods for reconstructing 256-byte sectors from the
342 XOR-decoded 6-bit values in a 6-and-2 data field:
  A: Boot ROM exact (decoded[86+Y]<<2 | decoded[Y]&3) -- simplest, used by ROM
  B: Old formula with reversed auxiliary index and shift
  C: Reversed aux index, 3-group packing (aux[85-i] covers bytes i, i+86, i+172)
  D: Forward aux index, 3-group packing
  E: No XOR chain at all (raw GCR values fed to boot ROM formula)

Applies each method to the D5 AA 96 sector 0 on track 0 and shows which
produce valid 6502 boot code (byte 0 as sector count, reasonable opcodes).

Usage:
    python woz_62fix.py

    Default path (override by editing APPLE_PANIC):
        woz_path - apple-panic/Apple Panic - Disk 1, Side A.woz

Output:
    Address field decode, GCR validity check, XOR chain checksum, decoded
    auxiliary and main buffer previews, first 32 bytes for each of the 5
    reconstruction methods, opcode analysis of byte 0, full hex dump for
    Method A, and quick disassembly from $0801 for the most promising methods.
"""
import struct
...
def main():
    """Decode the D5 AA 96 sector using 5 different reconstruction methods and compare."""
