#!/usr/bin/env python3
"""
Deep analysis of the Apple Panic copy protection scheme.

Performs a comprehensive multi-section analysis of the WOZ disk: disk structure
overview, per-track prologue detection (standard D5 AA 96 vs. custom D5 xx B5),
boot sector decode and hex dump, sync-byte/gap analysis, address field format
per track, 6-and-2 data nibble validity checking, epilogue format verification,
and sector ordering summary.
...
Output:
    An 8-section report printed to stdout covering disk structure, per-track
    analysis, boot sector disassembly, sync byte statistics, address field
    format, data encoding validity, epilogue format, and sector ordering.
"""
import struct
...
def to_nibbles(track_data, bit_count):
    """Convert raw track bytes to nibbles. Returns early once bit_count bits are consumed."""
    bits = []
    for b in track_data:
...
def main():
    """Run the full 8-section copy-protection analysis and print the report."""
