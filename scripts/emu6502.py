...
Provides a full NMOS 6502 CPU emulator (including undocumented opcodes), a
WOZ2 disk image reader with nibble streaming for emulated Disk II I/O, and
helper functions for decoding 6-and-2 and 5-and-3 GCR sectors.

Emulates enough Apple II hardware (disk controller soft switches, keyboard
strobe) to boot the copy-protected Apple Panic disk through all stages and
capture decrypted game code from memory.

Classes:
    WOZDisk  -- WOZ2 disk image parser and nibble streamer
    CPU6502  -- NMOS 6502 CPU with Apple II I/O hooks
...
Usage:
    ...
    Paths default to repo-relative locations (apple-panic/ for inputs,
    apple-panic/output/ for generated files).
...
class CPU6502:
    ...
    def _get_p(self):
        """Pack individual flag bits into a status register byte (NV.BDIZC)."""
        return (self.C | (self.Z << 1) | (self.I << 2) | (self.D << 3) |
                (1 << 4) | (1 << 5) | (self.V << 6) | (self.N << 7))

    def _set_p(self, val):
        """Unpack a status register byte into individual flag bits."""
        self.C = val & 1
        self.Z = (val >> 1) & 1
        self.I = (val >> 2) & 1
        self.D = (val >> 3) & 1
    ...
    def _set_nz(self, val):
        """Update N and Z flags from an 8-bit result, return masked value."""
        val &= 0xFF
        self.N = (val >> 7) & 1
    ...
    def read(self, addr):
        """Read a byte from memory, handling Disk II and keyboard soft switches."""
        addr &= 0xFFFF
    ...
    def write(self, addr, val):
        """Write a byte to memory, routing disk controller accesses to soft switches."""
        addr &= 0xFFFF
        val &= 0xFF
    ...
    def push(self, val):
        """Push a byte onto the 6502 stack (page $01, descending)."""
        self.mem[0x0100 + self.sp] = val & 0xFF
        self.sp = (self.sp - 1) & 0xFF

    def pull(self):
        """Pull a byte from the 6502 stack (page $01, ascending)."""
        self.sp = (self.sp + 1) & 0xFF
    ...
    def _addr_ind(self):
        """Resolve indirect addressing with the NMOS 6502 page-boundary bug."""
        lo = self.mem[(self.pc + 1) & 0xFFFF]
        hi = self.mem[(self.pc + 2) & 0xFFFF]
        addr = lo | (hi << 8)
        # NMOS 6502 bug: if lo byte is $FF, high byte wraps within same page
    ...
    def _op_adc(self, mode):
        """Add with carry, supporting both binary and BCD (decimal) modes."""
    ...
    def _op_sbc(self, mode):
        """Subtract with carry (borrow), supporting both binary and BCD modes."""
        val = self._resolve_read(mode)
        if self.D:
    ...
    def _branch(self, cond):
        """Execute a conditional branch: advance PC by 2, then add signed offset if cond is true."""
    ...
    def format_state(self):
        """Return a human-readable string of the current CPU register state."""
    ...
    def format_instr(self):
        """Return a formatted disassembly string for the instruction at PC."""
    ...
    def step(self):
        ...
        Returns:
            True if execution should continue, False if the CPU is halted.
        """
        if self.halted:
            return False
