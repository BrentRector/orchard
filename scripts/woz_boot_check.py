...
Scans the entire track 0 nibble stream for every D5-prefixed address prologue
(both standard D5 AA 96 16-sector and custom D5 xx B5 13-sector formats).
Decodes D5 AA 96 data fields using 6-and-2 encoding and reports validity counts
for both encoding schemes. Provides full hex dumps of the boot sector.

Usage:
    python woz_boot_check.py

    Default path (override by editing APPLE_PANIC):
        woz_path - apple-panic/Apple Panic - Disk 1, Side A.woz

Output:
    A list of every address field found on track 0 (type, position, volume,
    track, sector, checksum), 6-and-2 decoded sector data for D5 AA 96
    sectors, a summary count of each address type, and a full hex dump of
    the D5 AA 96 sector 0.
"""
import struct
...
def main():
    """Scan track 0 for all address fields and decode their data sectors."""
