#!/usr/bin/env python3
"""
Quick test of P5A 5-and-3 secondary buffer orderings.

Tests the P5A ROM reconstruction algorithm with both reversed and forward
secondary buffer orderings, and also tries forward iteration (i from 0 to 50
instead of 50 to 0). For each combination, compares the decoded sectors against
the cracked runtime image to find which ordering produces the best matches.

Usage:
    python woz_p5a_test.py
...
Output:
    Per-sector secondary buffer statistics, decoded first-16-byte previews
    for each ordering variant, $FF byte counts, and best-match scores against
    the cracked version (exact or N/256 with offset).
"""
import struct
...
def get_53_xor_decoded(nibbles, idx):
    """Translate 411 nibbles through the 5-and-3 table and XOR-chain decode them."""
    translated = []
    for i in range(411):
        nib = nibbles[idx + i]
...
def reconstruct(decoded, reverse_sec=True):
    """Reconstruct 256 bytes from 410 decoded 5-bit values using P5A algorithm.

    Args:
        decoded: List of 410 XOR-decoded 5-bit values.
        reverse_sec: If True, reverse the secondary buffer (standard P5A behavior).
    """
    thr = [0] * 154
    if reverse_sec:
        for j in range(154):
...
def main():
    """Test reversed vs. forward secondary ordering and iteration direction."""
