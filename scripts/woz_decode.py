...
Handles the custom disk format used by Apple Panic: 14 tracks with 13 sectors
each, non-standard D5 xx B5 address prologues, and standard D5 AA AD data
fields using 6-and-2 encoding. Decodes all sectors, writes a raw sequential
binary dump, and searches for known game signatures (JMP targets, text strings).

Usage:
    python woz_decode.py

    Default paths (override by editing APPLE_PANIC / OUTPUT_DIR):
        woz_path - apple-panic/Apple Panic - Disk 1, Side A.woz
        out_path - apple-panic/output/ApplePanic_original_raw.bin

Output:
    Per-track decode summary, sector-level hex previews, total byte counts,
    per-track MD5 hashes, and results of pattern searches for game code
    signatures. Writes the raw sector dump to out_path.
"""
import os
import struct
import hashlib
...
def read_woz2(path):
    ...
    for i in range(160):
        offset = 256 + i * 8
        sb = struct.unpack_from('<H', data, offset)[0]   # starting 512-byte block
        bc = struct.unpack_from('<H', data, offset + 2)[0]  # block count
        bits = struct.unpack_from('<I', data, offset + 4)[0]  # bit count
        if sb == 0 and bc == 0:
            continue
        tracks[i] = {
            'bit_count': bits,
...
def main():
    """Decode all 14 tracks of the copy-protected WOZ disk and dump sector data."""
    ...
    if all_sectors:
        ...
        out_path = str(OUTPUT_DIR / "ApplePanic_original_raw.bin")
