#!/usr/bin/env python3
"""
Deep debug of 6-and-2 decode from a WOZ2 file.

Low-level diagnostic tool that traces every step of the 6-and-2 decode process
for the D5 AA 96 sector 0 on track 0: WOZ2 header parsing, TMAP/TRKS chunk
inspection, bit-to-nibble conversion, address field decode with checksum
verification, gap nibble display, data field location, GCR translation, XOR
chain decode (forward and reverse), and multiple byte reconstruction methods
(boot ROM exact, packed 3-group forward, packed 3-group reversed, no-XOR).
Includes a quick disassembly from $0801 for each method.

Usage:
    python woz_62debug.py

    Default path (override by editing APPLE_PANIC):
        woz_path - apple-panic/Apple Panic - Disk 1, Side A.woz

Output:
    Extremely detailed trace of the decode pipeline: WOZ2 header fields, track
    entry metadata, nibble counts, raw address field bytes, gap nibbles, raw
    disk nibbles, GCR-decoded values, XOR chain results, multiple reconstruction
    results with hex dumps, and quick disassemblies.
"""
import struct
...
def main():
    """Trace the full 6-and-2 decode pipeline for the D5 AA 96 sector 0 on track 0."""
