#!/usr/bin/env python3
"""
Debug the 5-and-3 sector decode by inspecting intermediate values at each stage.

Reads sector 0 from Track 0 using both the original and corrupted GCR tables,
then shows the intermediate secondary/primary values before and after post-decode.
Verifies that the $02D1 post-decode produces the expected stage 2 code, and tests
the $0346 post-decode with corrupted table data.

This is a diagnostic script used during development to understand and validate
the two different post-decode routines ($02D1 for the initial boot sector and
$0346 for subsequent sectors loaded by stage 2).
...
Expected output:
    GCR table entries (corrupted vs original), sector 0 raw decode with both
    tables, secondary/primary value samples, $02D1 post-decode verification
    against expected stage 2 bytes, and $0346 post-decode result.
"""
import struct
...
def main():
    """Debug sector 0 decode with both original and corrupted GCR tables."""
