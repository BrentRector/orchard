#!/usr/bin/env python3
"""
Try multiple 5-and-3 decode variants to find the correct byte reconstruction.

Tests 8 different reconstruction strategies to determine how the original
disk controller processes 5-and-3 encoded data:
  V1: Continuous XOR chain, reversed secondary buffer
  V2: Split XOR chains (reset at secondary/primary boundary), reversed secondary
  V3: Continuous XOR, forward secondary (no reversal)
  V4: Split XOR, forward secondary
  V5: Primary-first layout (decoded[0..255]=top, [256..409]=thr), reversed
  V6: Primary-first, forward secondary
  V7: Raw translated values without byte reconstruction
  V8: Raw translated values without XOR decoding

Each variant is tested on sectors 0, 2, 3, and 12 and compared against the
cracked runtime image to find the best match.

Usage:
    python woz_53_variants.py
...
Output:
    Per-sector, per-variant first-8-byte previews, $FF counts, and best match
    scores against the cracked version (with memory offset).
"""
import struct
...
def main():
    """Test all 8 reconstruction variants and report match quality against cracked image."""
