#!/usr/bin/env python3
"""
Load all 5-and-3 GCR decoded sectors from Track 0 and analyze the boot flow.

The P5A (13-sector) boot ROM loads S0 into $0800, uses byte 0 as the sector
count, loads subsequent sectors into $0800+N*$100, then JMPs to $0801. This
script decodes all 13 sectors from the WOZ image, shows their memory layout,
provides a simple disassembly of the boot entry point ($0801), and compares
sector contents against a cracked runtime binary to identify code regions.

Usage:
    python woz_53boot.py

    Default paths (override by editing APPLE_PANIC):
        - .woz file: apple-panic/Apple Panic - Disk 1, Side A.woz
        - runtime binary: apple-panic/ApplePanic_runtime.bin

Expected output:
    Per-sector decode status (checksum, non-zero bytes), Sector 0 hex dump,
    memory map of all sectors, disassembly from $0801, and a similarity
    comparison of each sector against the cracked runtime binary.
"""
import struct
from pathlib import Path

# 5-and-3 GCR encoding table: 32 valid disk nibble values for the older
# 13-sector format used by Apple II before DOS 3.3's 6-and-2 scheme.
ENCODE_53 = [
...
# Reverse lookup: disk nibble -> 5-bit decoded value
...
def read_woz_nibbles(woz_path, track_num):
    ...
    Returns:
        A list of nibble bytes for the requested track.
    """
...
def decode_53_sector(nibbles, idx):
    """Decode a 5-and-3 data field (410 data + 1 checksum nibble).

    Implements the P5A ROM decode algorithm: GCR translate, XOR chain,
    then reconstruct 256 bytes from 154 secondary + 256 primary values
    using groups of 51 with 5-and-3 bit packing.

    Args:
        nibbles: Full nibble stream (with wrap extension).
        idx: Index of the first data nibble (after D5 AA AD prologue).

    Returns:
        Tuple of (256-byte sector data, checksum_ok boolean), or None on error.
    """
    GRP = 51  # Group size: 5 groups of 51 = 255, plus 1 final byte = 256
...
def decode_44(b1, b2):
    ...
    The Disk II encodes each address-field byte as two nibbles using
    4-and-4 encoding: odd bits in b1, even bits in b2.
    """
    return ((b1 << 1) | 1) & b2


def main():
    """Decode all 5-and-3 sectors from Track 0 and analyze the boot chain."""
