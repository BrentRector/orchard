#!/usr/bin/env python3
"""
Disassemble the Apple Panic boot sector (Track 0, Sector 0) as 6502 code.

Contains a hardcoded copy of the boot sector bytes (verified against the .dsk
file) and a 6502 disassembler with support for undocumented opcodes. The boot
sector loads at $0800 and is the first code executed by the Disk II boot ROM.

Usage:
    python woz_disasm_boot.py

Expected output:
    Full 6502 disassembly of the boot sector from $0800, plus a hex dump of
    the embedded data table region ($0844-$085C). Covers both Autostart ROM
    ($0801 entry) and Integer ROM ($0800 entry) interpretations.
"""
...
def disasm(data, base, start=0, end=None):
    """Disassemble 6502 machine code into human-readable assembly.

    Args:
        data: Byte sequence containing the machine code.
        base: The memory address that data[0] corresponds to.
        start: Offset within data to begin disassembly (default 0).
        end: Offset within data to stop disassembly (default len(data)).

    Returns:
        A list of formatted disassembly strings, one per instruction.
    """
    if end is None:
        end = len(data)
    pc = start
...
def main():
    """Disassemble the boot sector from $0800 and show the data table region."""
