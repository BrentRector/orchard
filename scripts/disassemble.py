#!/usr/bin/env python3
"""
Apple Panic (1981) - Full Disassembler producing Merlin32-syntax assembly source.

Reads the runtime memory image (produced by build_runtime.py) and generates a
complete, reassemblable Merlin32 assembly source file. Uses recursive descent
tracing from known entry points to classify code vs. data, then fills gaps with
speculative probing. Includes over 200 descriptive label names for subroutines,
jump targets, and data tables, plus automatic hardware register comments and
Apple II ROM call annotations.

The output is organized into sections:
  - Header with ORG directives
  - Pre-game data ($6000-$6FFF): sprite tables, level data, shape data
  - Main game code ($7000-$A7FF): traced and gap-filled
  - Clean loader ($1900-$1939): post-relocation startup stub

Usage:
    python disassemble.py [--input PATH] [--output PATH]

    Defaults use repo-relative paths; override with CLI arguments.

Output:
    Writes a Merlin32-compatible .asm file. Prints pass count, total code
    bytes found, and the output path to stdout.
"""
...
def load_image(path):
    """Load a raw binary file into a mutable bytearray."""
    with open(path, 'rb') as f:
        return bytearray(f.read())

def is_branch(mnemonic):
    """Return True if the mnemonic is a conditional branch instruction."""
...
class Disassembler:
    """6502 recursive-descent disassembler with code/data classification.

    Traces from entry points, follows branches/JSR/JMP, classifies addresses
    as code or data, assigns labels, and formats Merlin32-syntax output.
    """
    def __init__(self, mem, start=0, end=None):
        """Initialize the disassembler for the given memory image and address range."""
        self.mem = mem
        self.start = start
...
def main():
    """Load the runtime image and generate the full Merlin32 assembly source."""
    repo_root = Path(__file__).resolve().parent.parent
    parser = argparse.ArgumentParser(description="Disassemble Apple Panic runtime image to Merlin32 source.")
    parser.add_argument("--input", default=str(repo_root / "apple-panic" / "ApplePanic_runtime.bin"),
                        help="Path to the runtime memory image")
    parser.add_argument("--output", default=str(repo_root / "apple-panic" / "ApplePanic.asm"),
                        help="Path for the .asm output file")
    args = parser.parse_args()

    os.makedirs(os.path.dirname(os.path.abspath(args.output)), exist_ok=True)

    mem = load_image(args.input)
    print(f"Loaded runtime image: {len(mem)} bytes")

    code, data, labels = generate_source(mem, args.output)
    print(f"\nSource generated: {args.output}")
