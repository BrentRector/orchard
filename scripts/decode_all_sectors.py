#!/usr/bin/env python3
"""
Decode ALL 5-and-3 sectors from the Apple Panic WOZ disk image (Track 0).

Uses the corrupted GCR table (ASL x3, as stage 2 does) and the $0346
post-decode routine to reconstruct full 256-byte sectors. Saves individual
sector binaries and a combined memory image ($B600-$BFFF) to disk.
...
Usage:
    python decode_all_sectors.py

    Paths default to repo-relative locations (apple-panic/ for inputs,
    apple-panic/output/ for generated files).

Expected output:
    Per-sector hex dumps with ASCII, disassembly of code-like sectors,
    individual .bin files per sector, a combined $B600-$BFFF memory image,
    and a boot chain summary.
"""
...
def main():
    """Decode all Track 0 sectors and save binaries plus disassembly."""
    ...
    mem_path = str(OUTPUT_DIR / "decoded_sectors/stage2_memory_B600.bin")
