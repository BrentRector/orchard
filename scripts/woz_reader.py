#!/usr/bin/env python3
"""
WOZ2 disk image reader and DOS 3.3 disk extractor for Apple II.

Reads a WOZ2 flux-level disk image, decodes standard 6-and-2 encoded sectors,
builds a 140K DOS 3.3 disk image (.dsk), reads the VTOC and file catalog,
and extracts individual files (binary, Applesoft, Integer BASIC).

Usage:
    python woz_reader.py

    Default paths (override by editing APPLE_PANIC / OUTPUT_DIR):
        woz_path  - apple-panic/Apple Panic - Disk 1, Side A.woz
        dsk_path  - apple-panic/ApplePanic_original.dsk
        out_path  - prefix for extracted binary files (apple-panic/output/original_)

Output:
    Prints WOZ2 header info, TMAP track mapping, per-track decode status,
    VTOC/catalog listing, and extracted file details. Writes a .dsk image
    and individual binary files to disk.
"""
...
def main():
    """Read a WOZ2 image, build a DOS 3.3 .dsk, and extract all catalog files."""
    ...
    for f_info in files:
        ...
        if f_info['type'] == 'B':  # Binary file
            ...
            if len(data) >= 4:
                ...
                out_path = str(OUTPUT_DIR / f"original_{safe_name}.bin")
