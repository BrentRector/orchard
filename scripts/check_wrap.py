#!/usr/bin/env python3
"""
Check if sector 1's data spans the track wrap boundary in the WOZ bit stream.

Apple Panic's copy protection places sector 1's data field across the point
where the track's bit stream wraps around. Naive nibble extraction (which
discards leftover bits at the wrap point) produces corrupted data for this
sector. This script tests whether doubling the bit stream before nibble
extraction produces correct checksums and valid decoded data.

Compares sector 1 decoded from the bit-doubled stream vs the naive stream,
showing exactly how many bytes differ and whether the wrap-fixed version
looks like valid 6502 code.
...
Expected output:
    Track bit count, nibble counts for single vs doubled bit stream, sector 1
    occurrences with checksums (original and corrupted tables), decoded first
    32 bytes, code-likeness score, and a diff count between wrap-fixed and
    naive decodes.
"""
...
def main():
    """Test sector 1 decode with bit-level wrap handling vs naive approach."""
    REPO_ROOT = Path(__file__).resolve().parent.parent
    APPLE_PANIC = REPO_ROOT / "apple-panic"
    OUTPUT_DIR = APPLE_PANIC / "output"
    with open(str(APPLE_PANIC / 'Apple Panic - Disk 1, Side A.woz'), 'rb') as f:
        data = f.read()
    tmap = data[88:88+160]
    tidx = tmap[0]
    offset = 256 + tidx * 8
    ...
    if len(sec1_finds) >= 1:
        ...
        if diffs > 0:
            ...
            out_dir = str(OUTPUT_DIR / "decoded_sectors")
            os.makedirs(out_dir, exist_ok=True)
            with open(str(OUTPUT_DIR / "decoded_sectors/track00_sec01_wrap_fixed.bin"), "wb") as f:
                f.write(bytes(perm))
