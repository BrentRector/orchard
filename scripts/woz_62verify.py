#!/usr/bin/env python3
"""
Verify the corrected 6-and-2 GCR decode algorithm by comparing its output
against the known-good .dsk file contents.

Unlike woz_62compare.py which uses a naive decode, this script implements the
correct algorithm with reversed aux bytes and group-of-3 bit packing that
matches the actual Disk II P6 boot ROM behavior. A perfect 256/256 byte match
confirms the decode is correct.

Usage:
    python woz_62verify.py
...
Expected output:
    "PERFECT MATCH!" if the decode is correct, followed by a hex dump of the
    verified boot sector. Otherwise, a list of differing byte positions.
"""
import struct
from pathlib import Path

# 6-and-2 GCR encoding table: 64 valid disk nibble values
ENCODE_62 = [
...
def main():
    """Decode WOZ T0S0 with the corrected algorithm and compare to .dsk."""
    ...
    # Read WOZ and decode
    nibbles = read_woz_nibbles(str(APPLE_PANIC / "Apple Panic - Disk 1, Side A.woz"), 0)
    # Extend nibbles to handle wrap-around when searching for data fields
    nib = nibbles + nibbles[:2000]

    # Search for D5 AA 96 address prologue (6-and-2 sector header)
