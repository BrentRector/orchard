...
Compares the cracked runtime image against the raw WOZ sector dump to locate
game code. Tries direct byte-pattern matching, single-byte XOR decryption
(all 256 keys), 5-byte short-pattern scanning, and sector-level exact/XOR
matching. Also performs JMP/JSR target frequency analysis and per-byte XOR
key detection (single-byte and two-byte repeating keys).

Usage:
    python woz_compare.py

    Default paths (override by editing APPLE_PANIC / OUTPUT_DIR):
        apple-panic/ApplePanic_runtime.bin       - cracked runtime image (from build_runtime.py)
        apple-panic/output/ApplePanic_original_raw.bin - raw WOZ sector dump (from woz_decode.py)

Output:
    Prints pattern match results (direct, XOR, short), sector-level matches,
    JMP/JSR target frequency tables, and per-byte XOR key analysis.
"""
import hashlib
from pathlib import Path

def main():
    """Compare cracked runtime image against raw WOZ decode to find game code."""
    ...
    with open(str(OUTPUT_DIR / "ApplePanic_original_raw.bin"), 'rb') as f:
