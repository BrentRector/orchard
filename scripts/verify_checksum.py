...
Apple Panic's copy protection deliberately corrupts the GCR decode table
(ASL x3 on entries $99-$FF) during stage 2 of the boot process. This script
tests whether sector checksums pass with the original table, the corrupted
table, or both, to determine if checksum failures indicate copy protection
or a decoding bug.

Also simulates the exact boot code read algorithm (which uses the GCR table
as both decode table and temporary storage) to test self-modifying behavior.
...
Expected output:
    GCR table comparison (original vs corrupted entries), per-sector checksum
    results with both tables, and boot-code-exact simulation results showing
    which table produces valid checksums.
"""
import struct
...
def main():
    """Test sector checksums with original and corrupted GCR tables."""
