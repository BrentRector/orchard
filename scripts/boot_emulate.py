#!/usr/bin/env python3
"""
Emulate the Apple Panic boot process through stage 1 using emu6502.py.

Traces the boot chain from the 6-and-2 boot sector at $0801 through the
stage 2 loader, loading 5-and-3 sectors 0-9 into $B600-$BFFF, until the
final JMP $B700. Detects and reports key milestones (code relocation, GCR
table corruption, sector reads, checksum failures) during execution.

Saves a full 64K memory dump and the $B600-$BFFF region upon reaching $B700.

Usage:
    python boot_emulate.py [--woz PATH] [--output-dir DIR]

    Defaults use repo-relative paths; override with CLI arguments.

Expected output:
    Milestone trace messages (relocated to $0200, entered stage 2, sector
    loading progress), final CPU state, and saved memory dump files. Stops
    at JMP $B700 or after 10M instructions / 100 checksum failures.
"""
import argparse
import os
import sys
...
def main():
    """Run the stage 1 boot emulation from $0801 through JMP $B700."""
    repo_root = Path(__file__).resolve().parent.parent
    parser = argparse.ArgumentParser(description="Emulate Apple Panic boot through stage 1.")
    ...
    for i in range(max_instr):
        ...
        if pc == 0x0343 and cpu.mem[0x3E] == 0x00 and cpu.mem[0x3F] == 0xB7:
            ...
            with open(os.path.join(output_dir, "emu_boot_memory.bin"), 'wb') as f:
                f.write(cpu.mem)
            print(f"\n  Memory dump saved to emu_boot_memory.bin")

            # Save $B600-$BFFF
            with open(os.path.join(output_dir, "emu_B600_BFFF.bin"), 'wb') as f:
