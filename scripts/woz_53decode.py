...
The copy-protected disk uses D5 AA B5 address fields and 5-and-3 data encoding,
NOT the 6-and-2 encoding used by standard 16-sector disks. This script
implements three different byte-reconstruction variants (v1, v2, v3) for the
5-and-3 secondary buffer packing, since the exact algorithm used by the
protection scheme is unknown. Each variant is tried on every sector and
compared against the cracked runtime image to identify the correct one.

Usage:
    python woz_53decode.py
...
Output:
    Per-track address/data field counts, decoded sector previews for all three
    reconstruction variants, boot sector analysis, and pattern search results
    (JMP targets, Apple II soft-switch accesses, sector-level exact matches
    against the cracked version).
"""
import struct
import hashlib
...
def decode_53_raw(nibbles, idx):
    """Translate and XOR-decode 411 nibbles, returning raw 5-bit values for analysis."""
    if idx + 411 > len(nibbles):
        return None
...
def main():
    """Decode all tracks using 5-and-3 encoding and compare against cracked version."""
