#!/usr/bin/env python3
"""
Analyze the boot sector from the Apple Panic WOZ disk image.

Reads the raw decoded sector dump (produced by woz_decode.py), disassembles
the boot sector (Track 0, Sector 0) and its continuation (Sector 1), and
searches all decoded data for known game code patterns such as JMP $7465.
Also performs per-track byte frequency analysis to detect possible XOR
encryption.

Usage:
    python woz_boot.py

    Default path (override by editing OUTPUT_DIR):
        Reads apple-panic/output/ApplePanic_original_raw.bin (output of woz_decode.py)

Output:
    Prints boot sector hex dump, 6502 disassembly from $0801, per-sector
    first-16-byte preview for Track 0, game pattern search results, and
    per-track most-common-byte histograms.
"""
import struct
...
def main():
    """Disassemble the boot sector and search for game code patterns."""
