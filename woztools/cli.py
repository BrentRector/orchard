...
def parse_addr(s):
    """Parse a hex address like '0x4000' or '$4000' or '4000'."""
...
def cmd_scan(args):
    ...
    for track in range(35):
        ...
        # Try non-standard prologs if nothing found
        notes = []
        if not sec_62:
            alt = find_sectors_62(nibbles, addr_prolog=(0xDE, 0xAA, 0x96))
            if alt:
                sec_62 = alt
                notes.append("addr=$DE,6+2")
        if not sec_53:
            # Try with $DE first byte, wildcard third byte
            alt = find_sectors_53(nibbles, addr_prolog=(0xDE, 0xAA, None))
            if alt:
                sec_53 = alt
                notes.append("addr=$DE,5+3")
...
def cmd_decode(args):
    ...
    from .woz import WOZFile
    from .gcr import find_sectors_62, find_sectors_53
    ...
    # Also try non-standard prologs
    if not sec_62:
        sec_62 = find_sectors_62(nibbles, addr_prolog=(0xDE, 0xAA, 0x96))
