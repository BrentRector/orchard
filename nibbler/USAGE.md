...
| `nibbles` | Dump raw nibbles for a track with byte highlighting|
...
Scans each track for 6-and-2 (16-sector) and 5-and-3 (13-sector) encoded sectors. Reports encoding type, sector counts, and address/data checksum status. Automatically detects non-standard address prologs using structural pattern matching (third byte `$96`/`$B5`, 4-and-4 address data, nearby `D5 AA AD` data prolog).
...
- **Notes** — `addr=$D5 $BE $B5` means a non-standard address prolog was auto-detected. The three hex values are the prolog bytes (first byte, per-track second byte, format identifier)
...
- **Non-standard prologs** — the disk uses custom address markers instead of standard `D5 AA 96`/`D5 AA B5`
...
7. **Custom Address Field Second Byte** — per-track variations in the second prolog byte (standard is $AA)
8. **Non-Standard Sector/Track Numbers** — address fields with out-of-range values

---

## nibbles — Raw Nibble Dump

Dumps the raw nibble stream for a track, 32 nibbles per line. Useful for manual inspection of disk format, sync bytes, prologs, and gap patterns.

```
python -m nibbler nibbles <woz_file> <track> [--highlight D5,AA,96]
```

**Options:**
- `--highlight BYTES` — comma-separated hex byte values to bracket in the output (e.g., `D5,AA,96` to highlight standard address prolog bytes)

**Example:**

```
$ python -m nibbler nibbles "Apple Panic - Disk 1, Side A.woz" 0 --highlight D5,AA

Track 0: 6388 nibbles

      0: FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF [D5] [AA] B5 BF BF FF FE FF AD AB FF FE
     32: [D5] [AA] AD AB AE AE ...
```

**What to look for:**
- **Sync bytes** — long runs of $FF before sector headers
- **Address prologs** — `D5 AA 96` (6-and-2), `D5 AA B5` (5-and-3), or custom variants
- **Data prologs** — `D5 AA AD` marking the start of sector data
- **Epilogs** — `DE AA EB` marking the end of address/data fields
- **Gap patterns** — non-standard gap bytes between sectors may indicate protection
...
- `--trace` — log disk I/O operations (seeks, address/data prolog detections) to stdout
...
from nibbler.gcr import find_sectors_62, find_sectors_53, auto_detect_address_prologs
...
| `gcr.py`     | `find_sectors_62()`, `find_sectors_53()`, `auto_detect_address_prologs()`, `scan_address_fields()`, `decode_sector_62()`, `decode_sector_53()`, GCR encode/decode tables |
| `cpu.py`     | `CPU6502` — full NMOS 6502 emulator (all 256 opcodes including 29 undocumented), breakpoint system, Disk II soft switch handling |
| `disk.py`    | `WOZDisk` — nibble streaming from WOZ bit streams, stepper motor simulation, I/O trace callbacks |
