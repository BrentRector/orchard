...
; Loaded from tracks 1-5 to $0800-$48FF by secondary loader at $B700
;
...
;
; LIFECYCLE:
; 1. Secondary loader at $B700 reads tracks 1-5 to $0800-$48FF
; 2. JSR $1000 — patches RWTS ($D5 -> $DE), enables graphics mode
; 3. Returns to secondary loader, which patches $129D to RTS
; 4. JSR $1290 — runs title screen display sequence (animation + sound)
; 5. Secondary loader reads tracks 6-13 to $4000-$A7FF, OVERWRITING
...
;
; COPY PROTECTION LAYER 7:
; The patcher at $1000-$1027 implements the $DE address marker scheme.
; After track 0 boots using standard $D5 AA xx address prologs, this
; code changes the RWTS to search for $DE AA xx on all subsequent
; tracks (1-13). Two RWTS locations are patched:
;   $B976: CMP #$D5 -> CMP #$DE (address field search)
;   $BEF5: LDA #$D5 -> LDA #$DE (address field write)
...
;
; Called via JSR $1000 from the secondary loader at $B745.
...
;
; Copy Protection Layer 7: The $DE marker patch means tracks 1-13 use
; non-standard address field prologs ($DE AA xx instead of $D5 AA xx).
; The per-track third byte is handled separately by the $B7CB routine.
; ============================================================================
            ORG $1000
...
; The secondary loader at $B759 calls JSR $1290 to run the title
...
; secondary loader at $B700, which:
