; ============================================================================
; Apple Panic -- Game Loader and RWTS ($B700-$BFFF)
; Disassembled from WOZ disk image (Apple Panic - Disk 1, Side A.woz)
;
; This file covers the game loader at $B700 and its supporting RWTS
...
;      ($B600 page is garbled: sector 0 re-read with corrupted GCR table)
;   3. JMP ($003E) = JMP $B700 (THIS FILE)
;      -> Copies per-track second-byte table from $B7E2 to $0400
;      -> Reads tracks 1-5 to $0800-$48FF (prologs: D5 [per-track] B5)
;      -> JSR $1000: patches RWTS first byte ($D5->$DE), then title screen
;      -> JSR $1290: title screen animation
;      -> Reads tracks 6-13 to $4000-$A7FF (prologs: DE [per-track] B5)
;      -> JMP $4000: game start (relocation + JMP $7000)
;   4. Game runs from $7000 (see ApplePanic.asm)
;
; MEMORY MAP ($B700-$BFFF, one page per track-0 sector):
;   ($B600-$B6FF is sector 0 re-read with corrupted GCR -- garbled, unused)
;   $B700-$B7FF  Sector 1: Game loader entry point + per-track table
...
;   Layer 7: Per-track second-byte table at $B7E2, copied to $0400
;            SUB_B7CB reads $0400[track] | $AA and patches $B980 and $BEFA
;            This varies the second byte of the address prolog per track
;   Layer 8: Address prolog first byte starts as $D5, gets patched to $DE
;            at runtime by $1000 code (patches $B976 and $BEF5)
;            Only affects tracks 6-13 (tracks 1-5 are read before the patch)
;
; Address prolog format:  [first] [second] B5
;   Tracks 0-5:  D5 [per-track] B5  (first byte unpatched)
;   Tracks 6-13: DE [per-track] B5  (first byte patched by $1000)
;
; Per-track address prolog second byte table:
...
;
; RUNTIME PATCHES (applied by $1000 intermediate code, BEFORE title screen):
...
;   $B980: CMP #$xx  (address field second byte check)
;   $BEFA: LDA #$xx  (address field second byte for writing)
;
...
; ============================================================================
; NOTE: $B600-$B6FF is NOT included in this file.
;
; The stage 2 loading loop reads sectors 0-9 into $B600-$BFFF. Sector 0 is
; re-read during this loop, but by this point the GCR table has been
; corrupted (ASL x3 on entries $99-$FF). The re-read decodes sector 0 with
; the wrong table, producing 256 bytes of garbled data at $B600. No code
; in the game or loader ever references $B600-$B6FF.
; ============================================================================


; ============================================================================
; $B700-$B7FF -- Game Loader Entry Point (Sector 1)
;
...
;   2. Copy 24-byte per-track second-byte table from $B7E2 to $0400
...
; The per-track second-byte table was placed at $B7E2 by the stage 2 loader
; when it read track 0 sector 1 to $B700. Each byte, OR'd with $AA, becomes
; the second address prolog byte for that track (copy protection layer 7).
...
; ============================================================================
; SET_TRACK_BYTE -- Patch address prolog second byte for current track
;
...
;
; This implements copy protection layer 7: each track has a different
; second byte in its address field prolog (D5/DE xx B5).
; ============================================================================
...
; ============================================================================
; Per-Track Second-Byte Table (24 bytes)
;
...
; After ORA #$AA they become the address prolog second bytes:
...
;
; Encodes 256 data bytes into 411 disk nibbles using the 5-and-3 GCR scheme.
...
;
; Not used during normal game boot (read-only), but present as part of
; the RWTS's format/write capabilities.
; ============================================================================
...
; for timing, loop through all 411 nibbles plus prolog/epilog markers.
...
; the pattern: D5 xx B5 (or DE xx B5 after $1000 patches $B976).
;
; The second byte (xx) is set per-track by SET_TRACK_BYTE at $B7CB, which
...
; and timing constants used by the disk read/write routines.
; ============================================================================
...
; game loader at $B7AC with the IOB address in A (high) and Y (low).
;
; On entry:
...
; ============================================================================
; END OF GAME LOADER AND RWTS DISASSEMBLY
; ============================================================================
