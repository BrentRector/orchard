...
; 7. JMP ($003E) = JMP $B700 transfers to game loader
;
...
;   $020F-$0243  GCR table builder + first sector read + self-mod + JMP $0301
;   $0244-$025C  Unused padding (zeros)
...
;   $B600 = Sector 0   $B700 = Sector 1 (game loader entry)
...
;   After loading:  ($3E/$3F) = $B700 -> enters game loader
; ============================================================================
...
jmp_indirect_3e:
            JMP ($003E)          ; 0343: 6C 3E 00  -> JMP $B700 (game loader)
                                 ;                  (during loading: -> $025D)
...
;
; After this code completes, control transfers to the game loader at
; $B700 (loaded from track 0, sector 1).  The game loader reads
