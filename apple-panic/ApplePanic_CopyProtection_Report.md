...
> **Correction:** The disassembly below was produced before the 6-and-2 boot
> sector was properly decoded. The actual boot sector has sector count = $01
> (not $00), and the code is a straightforward relocation stub — no
> undocumented opcodes, no zero-page execution. The byte `$9C` does not appear
> anywhere in the boot code. See `ApplePanic_Boot_Stage2.asm` for the correct
> disassembly.
...
| 7 | Per-track second-byte variations in address prologs | Medium               |
| 8 | Non-standard $DE first byte on tracks 6-13          | Medium               |
